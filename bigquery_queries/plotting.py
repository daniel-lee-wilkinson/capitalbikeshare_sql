...
grouped = (
    df.groupby(["day_group", "start_lat", "start_lng", "member_casual"])["trip_count"]
    .mean()
    .reset_index()
)

grouped["trip_bin"] = pd.cut(
    grouped["trip_count"], bins=[0, 5, 15, 30, 50, float("inf")], labels=[1, 2, 3, 4, 5]
).astype(float)
...
for group in groups:
    for col, rider_type in enumerate(["member", "casual"], start=1):
        day_data = grouped[
            (grouped["day_group"] == group) & (grouped["member_casual"] == rider_type)
        ]
...
for i, group in enumerate(groups):
    ...
    buttons.append(
        dict(
            label=group,
            method="update",
            args=[{"visible": visibility}, {"title": f"Capital Bikeshare — {group}"}],
        )
    )
    ...
    return pd.DataFrame(
        {
            ...
            "day_group": ["1. Weekday (Mon–Thu)", "3. Saturday"],
            "start_lat": [38.9, 38.95],
            "start_lng": [-77.0, -77.05],
